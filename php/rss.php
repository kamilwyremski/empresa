...
foreach($offers->offer_data as $key=>$value){
	...
	$rssfeed .= '<link>'.$settings['base_url'].'/'.$value['id'].','.$value['slug'].'</link>';
	$rssfeed .= '<guid>'.$settings['base_url'].'/'.$value['id'].','.$value['slug'].'</guid>';
	...
	if($value['thumb']){
		$rssfeed .= '&lt;br&gt;&lt;br&gt;&lt;a href="'.$settings['base_url'].'/'.$value['id'].','.$value['slug'].'"&gt;&lt;img src="'.$settings['base_url'].'/upload/photos/'.$value['thumb'].'" height="80"/&gt;&lt;/&gt;';
	}
	$rssfeed .= '</description>';
    $rssfeed .= '</item>';
}
