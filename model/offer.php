...
if(isset($_GET['id']) and $_GET['id']>0 and $offer->checkActiveOffer($_GET['id'])){
	...
	if($_GET['slug']!=$offer->slug){
		header("Location: ".$settings['base_url'].'/'.$offer->id.','.$offer->slug);
		die('redirect');
	}
	...
	if(isset($_POST['action']) and $_POST['action']=='send_message' and isset($_POST['name']) and isset($_POST['email']) and isset($_POST['message']) and isset($_POST['captcha'])){
		...
		if(isset($error)){
			...
			if(send_mail('offer',$offer->email,array('name'=>$_POST['name'], 'email'=>$_POST['email'], 'message'=>$_POST['message'], 'offer_name'=>$offer->name, 'offer_url'=>$offer->id.','.$offer->slug))){
